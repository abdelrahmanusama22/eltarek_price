        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            @foreach($priceEntries as $entry)
                ...
                @foreach($activeOffers as $offer)
                    @php
                        $execPrice = floatval($entry->execution_price);
                        $val = floatval($offer['value'] ?? 0);
                        $type = $offer['offer_type'] ?? 'fixed';
                        $finalPrice = $type === 'percentage' ? $execPrice + ($execPrice * ($val / 100)) : $execPrice + $val;
                    @endphp
<div class="bg-white rounded-xl shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border border-red-100 overflow-hidden flex flex-col relative group">
    ...
    <div class="h-32 bg-gradient-to-br from-red-50 to-white flex items-center justify-center p-6 border-b border-red-50 relative">
        ...
        <div class="relative z-10 text-center">
            <h3 class="text-xl font-bold text-gray-900 leading-tight">{{ $entry->car?->model_sales_code ?? 'N/A' }}</h3>
            <p class="text-xs font-semibold text-primary mt-1 uppercase tracking-wider">{{ $entry->brand?->name ?? 'Unknown Brand' }} • {{ $entry->year }}</p>
        </div>
    </div>
    ...
    <div class="p-6 flex-grow flex flex-col">
        ...
        <div class="mb-4">
            ...
            @else
                <div class="flex items-end gap-2 mb-1">
                    <span class="text-3xl font-extrabold text-primary">{{ number_format($finalPrice, 2) }}</span>
                    ...
                    @foreach($entry->available_colors as $color)
                        <div class="group/color relative flex items-center">
                            <div class="w-6 h-6 rounded-full border-2 border-white shadow-sm ring-1 ring-gray-200" 
                                 style="background-color: {{ $color['color_code'] ?? '#ccc' }}"
    ...
    <div class="px-6 py-4 bg-gray-50 border-t border-gray-100">
        <a href="{{ route('sales.price', $entry->car_id) }}" class="w-full flex justify-center items-center px-4 py-2.5 border border-transparent text-sm font-bold rounded-lg shadow-sm text-white bg-gray-900 hover:bg-black focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-900 transition-colors">
